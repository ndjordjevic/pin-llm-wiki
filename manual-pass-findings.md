...
- **Synthesis pages created:** 0 — held off; no cross-source material yet to anchor a durable synthesis page
...
- `[DECISION]` Held synthesis pages until cross-source material arrives. "Subagent orchestration" and "TDD in agentic systems" are obvious candidates but need more than one source to justify a durable page.
...
`[REVISE]` **Self-referential `sources:` frontmatter on source pages** — agent wrote `sources: - "[[superpowers]]"` on `wiki/sources/superpowers.md`. `sources:` semantically means "source pages this synthesis draws from," which is not meaningful on a source page. Fix: CLAUDE.md must state that source pages MUST NOT include `sources:`. Research.md §6 frontmatter spec is ambiguous — worth tightening there too.

`[REVISE]` **Citation path format** — CLAUDE.md template showed root-relative (`[...](raw/github/...)`) but Obsidian and standard markdown both resolve link targets relative to the file's location, so root-relative breaks. rp6502-kb uses `../raw/...`. Fix: CLAUDE.md now specifies relative-from-file (`../../raw/...` from `wiki/sources/` and `../raw/...` from `wiki/syntheses/`).
...
- **Synthesis pages created:** 0 — deferred; cross-source patterns emerging (tool access scoping, human-in-the-loop policy) but single source not enough to anchor a synthesis page yet
...
`[OPEN]` **Cross-source synthesis pages** — clear patterns across all three sources: human-in-the-loop (all), tool access scoping (all), composable skills/capabilities (Superpowers + DeepAgents). Could become synthesis pages once the wiki needs permanent deep-dives beyond `overview.md`.
...
### Check 4: Data gaps (concepts mentioned but no synthesis page)

Three cross-source patterns are explicitly named in `overview.md` but have no dedicated synthesis pages:
...
| **Agent observability / evaluation** | LangSmith (1 source) | Single-source so far; synthesis page premature but gap noted |
| **Persistent vs session-based agents** | Paperclip heartbeat vs Superpowers session | Pattern difference not yet named as a synthesis |
| **TDD in agentic systems** | Superpowers (1 source) | Single-source; premature for synthesis but gap noted |

`[DECISION]` Synthesis pages were intentionally deferred during Steps 1–3 (insufficient cross-source material per Step 1 findings decision). By the end of the pass, tool access scoping, human-in-the-loop, and layered architecture looked strong enough for future synthesis pages if the wiki needed them.
...
| Data gaps | ⚠️ 3 ready, 6 noted | 3 strong synthesis candidates; 6 more candidates |
...
`[OPEN]` **Agent produced a better answer than the wiki's own overview** — the comparison table is more precise than what `overview.md` says about tool use. This raises a question: should the lint/fix pass after an agent-consumption test be used to harvest agent-generated insights back into synthesis pages? That's an emergent workflow not anticipated in the plan.
...
5. **Structured methodology over ad-hoc ingest.** Deferring synthesis pages until cross-source material justified them kept the wiki clean rather than filling it with premature abstractions.
...
2. **§6 frontmatter spec: tighten `sources:` field semantics.** `sources:` should only appear on synthesis pages (listing the source pages they draw from). Source pages must NOT include `sources:` — they ARE the source. Currently ambiguous.
...
1. **Synthesis pages are manual, not ingest-time artifacts.** Ingest creates source pages only. Cross-source synthesis requires enough evidence and editorial judgment to justify a page.
...
6. **Agent-consumption outputs are candidates for harvest back into synthesis pages.** When a fresh agent session produces a synthesis insight better than what's in the wiki, that insight should be written into the relevant synthesis page. This closes the loop: wiki improves over use.
...
3. ~~**Cross-source page creation trigger** — should lint suggest dedicated concept pages automatically?~~ **Resolved:** cross-source material lives in `overview.md` by default and in manual `wiki/syntheses/` pages when justified.
...
7. **Lint auto-fix scope** — which lint findings should be auto-fixable vs report-only? Candidates for auto-fix: add `overview.md`/`log.md` links to index, add `related:` cross-refs. Candidates for report-only: contradictions, citation gaps (require human judgment).
...
2. **§6 frontmatter spec → tighten `sources:` field** — source pages must NOT include `sources:`; that field is only for synthesis pages.
...
9. **New → add "agent-consumption harvest" workflow** — insights generated during agent-consumption tests are candidates to write back into synthesis pages.
